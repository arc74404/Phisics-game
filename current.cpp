...
void current::setPower(float p)
{
    this->power = p;
}
float current::getPower() const 
{ 
    return this->power;
}

void current::setDirection(float dd)
{
    direction = dd;
}
float current::getDirection() const
{
    return direction;
}
...
void current::setVector_coordinates(sf::Vector2f v)
{
    Vector_coordinates = v;
}

int artan(sf::Vector2f a)
{
    int corn = 0;
    if(a.x > 0 && a.y > 0)
    {
...
void current::Null()
{
    Vector_coordinates.x = 0; Vector_coordinates.y = 0;
    power = 0;
    direction = 0;
}
current current::operator+(const current& other)
 {
    ...
    sf::Vector2f help(this->Vector_coordinates * this->getPower() + other.Vector_coordinates * other.getPower());

    sum.setDirection(artan(help));
    sum.setVector_coordinates();

    float pp = sqrt(pow(help.x, 2) + pow(help.y, 2)) / sqrt(pow(sum.Vector_coordinates.x, 2) + pow(sum.Vector_coordinates.y, 2));
    sum.setPower(pp);
